---
layout: page
permalink: /design_pattern/
title: Design pattern
---

Page dedicated to knowledge related to design patterns.

- [Behavioral pattern](/wiki/design_pattern/behavioral_pattern)
    - [Bytecode](/wiki/design_pattern/behavioral_pattern/bytecode)
    - [Subclass](/wiki/design_pattern/behavioral_pattern/subclass)
    - [Type object](/wiki/design_pattern/behavioral_pattern/type_object)
- [Command](/wiki/design_pattern/command)
- [Decoupling pattern](/wiki/design_pattern/decoupling_pattern)
    - [Component](/wiki/design_pattern/decoupling_pattern/component)
    - [Service locator](/wiki/design_pattern/decoupling_pattern/service_locator)
    - [Subclass](/wiki/design_pattern/decoupling_pattern/event_queue)
- [Flyweight](/wiki/design_pattern/flyweight)
- [MVC](/wiki/design_pattern/mvc)
- [Observer](/wiki/design_pattern/observer)
- [Optimization patterns](/wiki/design_pattern/optimization_pattern)
    - [Data locality](/wiki/design_pattern/optimization_pattern/data_locality)
    - [Dirty flag](/wiki/design_pattern/optimization_pattern/dirty_flag)
    - [Object pool](/wiki/design_pattern/optimization_pattern/object_pool)
    - [Spatial partition](/wiki/design_pattern/optimization_pattern/spatial_partition)
- [Prototype](/wiki/design_pattern/prototype)
- [Sequencing patterns](/wiki/design_pattern/sequencing_pattern)
    - [Double buffering](/wiki/design_pattern/sequencing_pattern/doube_buffering)
    - [Game loop](/wiki/design_pattern/sequencing_pattern/game_loop)
    - [Update method](/wiki/design_pattern/sequencing_pattern/update_method)
- [Singleton](/wiki/design_pattern/singleton)
- [State](/wiki/design_pattern/state)


